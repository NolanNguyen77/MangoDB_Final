...
DROP VIEW  IF EXISTS agri.v_qr_public;
...
-- (5) QR code gắn lô
-- ON DELETE CASCADE: Xóa batch sẽ tự động xóa tất cả QR codes thuộc batch đó
CREATE TABLE agri.qrcode (
  qr_id      SERIAL PRIMARY KEY,
  batch_id   INT NOT NULL REFERENCES agri.batch(batch_id) ON DELETE CASCADE,
...
-- ON DELETE CASCADE: Xóa variety sẽ tự động xóa lịch sử giá của giống đó
CREATE TABLE agri.price_history (
  price_id   SERIAL PRIMARY KEY,
  variety_id INT NOT NULL REFERENCES agri.variety(variety_id) ON DELETE CASCADE,
...
-- (9) View tối giản cho UI
CREATE OR REPLACE VIEW agri.v_qr_public AS
SELECT
  q.code,
